...
import idv.hsu.taipeizoo.data.model.TaipeiZooResponse
...
class TaipeiZooRepositoryTest {
    ...
    private val repository: TaipeiZooRepository = object : TaipeiZooRepository {
        override fun getAllDataFlow(): Flow<idv.hsu.data.util.ApiResultWrapper<TaipeiZooResponse>> {
            return mockDataSource.getDataFlow()
        }

        override suspend fun getAllData(): idv.hsu.data.util.ApiResultWrapper<TaipeiZooResponse> {
    ...
    fun `test getAllData should return data`() = runTest {
        val mockApiResult = idv.hsu.data.util.ApiResultWrapper.Success(mockResponse)
    ...
    fun `test getAllDataFlow should emit data`() = runTest {
        val mockApiResultFlow = flowOf(idv.hsu.data.util.ApiResultWrapper.Success(mockResponse))
        coEvery { mockDataSource.getDataFlow() } returns mockApiResultFlow

        val resultFlow = repository.getAllDataFlow()

        resultFlow.collect { result ->
            assertEquals(idv.hsu.data.util.ApiResultWrapper.Success(mockResponse), result)
        }
